...
We want to exclude frond observations from the date 2000-09-22 in transect 6 in quad 40 because they are marked to have "-99999" fronds. This does not make biological sense, represents missing data, and therefore must be filtered out or may skew calculations. It is also outlined in the methods and protocols section that either this data was either not collected or was lost.
...
The response variable, nest site use vs non-use is a binary variable. 1 indicates a plover nest was actively used by a Mountain plover, while 0 indicates an unused site where no plovers were detected and no nests were observed which allows for the analysis of habitat characteristics.
...
*Distance to prairie dog colony edge* Distance to prairie dog colony edge (edgedist) is a continuous variable, as distance can also take on any value in a range including fractional units. Edgedist is measured by determining the distance from a given point to the nearest edge of the prairie dog colony. As distance to the prairie dog colony edge increases, the probability of Mountain Plover nest site use would decrease because they engineer suitable plover habitat through soil disturbance and vegetation clipping(introduction, paragraph 3). Colony edges have complementary resources and promote predator cover (discussion, paragraph 2). 

## c. Table of models

| Model Number | Visual obstruction | Distance to prairie dog colony | Model description           |
|--------------|--------------------|--------------------------------|-----------------------------|
| 0            | X                  | X                              | No predictors (null model)  |
| 1            | ✔                  | ✔                              | All predictors (full model) |
...
```{r}
#| label: clean data and run models
#| message: false
#| warning: false

mopl_clean <- mopl |> # set mopl to cleaned data object
  clean_names() |> # lowercase and underscore
     mutate(used_bin = case_when(
    id == "used" ~ 1, # assign 1 to used nest sites
    id == "available" ~ 0 # assign 0 to available but unused sites
  )) |>
  # keep only needed columns
  select(used_bin, edgedist, vo_nest)
mopl_model0 <- glm( used_bin ~ 1, # response
                    data = mopl_clean, # create a null glm model using cleaned mopl dataset
                    family = "binomial") # use logistic regression glm
                   
mopl_model1 <- glm(used_bin ~ edgedist + vo_nest, # response vs. edge distance and visual obstruction
                   data = mopl_clean,  # saturated glm model
                   family = "binomial") # use logistic regression type glm

mopl_model2 <- glm(used_bin ~ edgedist, # response vs. edge distance
                   data = mopl_clean, # create a glm model using clean mopl data
                   family = "binomial") # use logistic regression type glm
...
```{r}
AIC(mopl_model0, mopl_model1, mopl_model2, mopl_model3) # select the best fitting model based on Akaike's Information Criterion (AIC)
```
The best model as determined by Akaike’s Information Criterion (AIC) is the model using only visual obstruction as a predictor of nest site use (AIC = 331.77), as it has the lowest AIC value of the four candidate models.
...
```
The simulated residuals follow a uniform distribution, as shown visually by the points closely following the red theoretical line in the QQ plot. Additionally, the residuals vs model rank transformed predictions plot shows no obvious patterns, suggesting that all the model assumptions are met. This is the expected distribution for a  logistic regression model, unlike linear regression, where residuals should follow a normal distribution.
...
mopl_preds <- ggpredict(mopl_model3, # generate predicted probabilities 
                        terms = "vo_nest [all]") # predict across range of visual obstruction values
...
The predicted probability of Mountain Plover nest site occupancy at 0 cm visual obstruction is 73% (95% CI: 0.65, 0.80). Overall, the selected model shows a negative relationship between visual obstruction and probability of occupancy. As Figure 1 shows, as visual obstruction increases, there is a lower probability of plover nest sites being used with a sharp decrease around 5 cm and higher of obstruction. Biologically, reduced vegetation height and increased exposure of bare ground enhances the Mountain Plover habitat quality. Additionally, sparse vegetation in the range of 3–5 cm also creates ideal plover habitat because the layer of vegetation conceals their body while still allowing them to scan for and detect approaching predators (Discussion, paragraph 4). 
...
**Check 1: Random Sample** Each data point represents an independently recorded study session, and sessions were not deliberately selected to favor a location. Location selection was haphazard, so this assumption is met.
...
I found a small (Cliff’s δ = -0.08) difference in median task completed between cafés (n = 15) and the library(n = 15). (Wilcoxon rank-sum test, W = 103.5, p = 0.72, ⍺ = 0.05). Tasks completed tended to be smaller in cafés (median = 3) than in the library(median = 5).
...
ggplot(data = my_clean_data,
  ...
  scale_y_continuous(breaks = seq(0, 14, by = 2)) + # add line breaks 
